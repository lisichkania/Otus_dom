...
#### 1. Создаём 4-ю ВМ на ЯО и устанавливаем postgres 15.
```
otus@otus-db-pg-vm-4:~$ sudo apt update && sudo apt upgrade -y -q && sudo sh -c 'echo "deb http://apt.postgresql.org/pub/repos/apt $(lsb_release -cs)-pgdg main" > /etc/apt/sources.list.d/pgdg.list' && wget --quiet -O - https://www.postgresql.org/media/keys/ACCC4CF8.asc | sudo apt-key add - && sudo apt-get update && sudo apt -y install postgresql-15
Hit:1 http://mirror.yandex.ru/ubuntu jammy InRelease
Get:2 http://mirror.yandex.ru/ubuntu jammy-updates InRelease [119 kB]
Get:3 http://mirror.yandex.ru/ubuntu jammy-backports InRelease [109 kB]
Get:4 http://security.ubuntu.com/ubuntu jammy-security InRelease [110 kB]
...
No VM guests are running outdated hypervisor (qemu) binaries on this host.
otus@otus-db-pg-vm-4:~$ pg_lsclusters
Ver Cluster Port Status Owner    Data directory              Log file
15  main    5432 online postgres /var/lib/postgresql/15/main /var/log/postgresql/postgresql-15-main.log
```
#### 2. Задаем пользователю postgres на 3-й ВМ пароль: ALTER USER postgres WITH PASSWORD 'otus123';
```
postgres=# \du
                                   List of roles
...
```
#### 3. Настраиваем мастер-сервис: 
```
otus@otus-vm-db-pg-net-3:~$ sudo nano /etc/postgresql/15/main/postgresql.conf
wal_level = hot_standby
otus@otus-vm-db-pg-net-3:~$ sudo nano /etc/postgresql/15/main/pg_hba.conf
# Allow replication connections from localhost, by a user with the
# replication privilege.
local   replication     all                                     peer
host    replication     postgres        51.250.19.226/32        md5
host    replication     all             127.0.0.1/32            scram-sha-256
host    replication     all             ::1/128                 scram-sha-256
```
#### 4. Конфигурация подчиненного сервера: 
```
otus@otus-db-pg-vm-4:~$ sudo nano /etc/postgresql/15/main/postgresql.conf
listen_addresses = 'localhost,51.250.19.226'
wal_level = hot_standby
hot_standby = on
```
#### 5. Синхронизируем данные с главного сервера на подчиненный сервер:
```
otus@otus-db-pg-vm-4:~$ sudo systemctl stop postgresql@15-main
otus@otus-db-pg-vm-4:~$ sudo rm -rf /var/lib/postgresql/15/main
otus@otus-db-pg-vm-4:~$ sudo -u postgres pg_basebackup -h 84.252.136.61 -p 5432 -R -D /var/lib/postgresql/15/main -U postgres -v -P
otus@otus-db-pg-vm-4:~$ sudo systemctl start postgresql@15-main
```
#### 6. Проверяем данные на подчиненном сервере просмотром test и test2:
```
postgres=# \dt
         List of relations
 Schema | Name  | Type  |  Owner
...
(10 rows)
```
#### 7. Вносим еще данных в таблицы test на 1-й ВМ и test2 на 2-й ВМ, проверяя их на 3-й и 4-й ВМ:
*1 ВМ*
```
postgres=# INSERT INTO test(i) SELECT generate_series(21, 30);
INSERT 0 10
postgres=# select * from test;
 i
----
...
*2 ВМ*
```
postgres=# INSERT INTO test2(i) SELECT generate_series(31, 40);
INSERT 0 10
postgres=#  select * from test2;
 i
----
...
*Горячее реплицирование для высокой доступности на 4ВМ работает*
